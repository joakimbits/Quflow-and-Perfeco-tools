...
public class TabbedLibraryEditor extends TabbedEditor {
  ...
  public JFileChooser getChooser () {
    if (chooser==null) { // It is a static chooser
      chooser=OSPRuntime.createChooser("java",new String[]{"java"},ejs.getSourceDirectory().getParentFile());
  ...
  protected void customMenuItems (java.awt.event.ActionListener actionListener) {
    ...
    addPage2 = createMenuItem("importExternalLibraryPage", defaultHeader, new ActionListener() {
      public void actionPerformed (ActionEvent _evt) {
        File javaFile = ejs.chooseFileUnderSource(getChooser(), finalPanel, true); // true to open
        ...
        else addPage(LIBRARY_EXTERNAL_EDITOR, FileUtils.getPlainName(javaFile), ejs.getRelativePath(javaFile), true);
      }
    });
    ...
    externalJavami.addActionListener (new ActionListener() {
      public void actionPerformed (ActionEvent _evt) {
        File javaFile = ejs.chooseFileUnderSource(getChooser(), finalPanel, true); // true to open
        if (javaFile==null) return; // The user canceled it
        if (!javaFile.exists()) JOptionPane.showMessageDialog(getComponent(), res.getString("Osejs.File.ReadError"),

...
public class EquationEditor implements Editor {
  ...
  private JComboBox forceSynchroCombo, estimateFirstStepCombo, useBestInterpolationCombo, accIndVelocityCombo;
  ...
  private boolean changed=false; //, reading=false;
  ...
  static {
    ...
    for (int j=0; j<solverNames.length; j++) {
      solverClassname[j] = sysRes.getString("EquationEditor."+solverNames[j]);
  ...
  public EquationEditor (Osejs _ejs, TabbedEvolutionEditor aParentEditor) {
    ...
    this.activeComponentSet.add(table);
    ...
    KeyListener keyListener = new KeyAdapter() {
      ...
      private void checkCorrectness(Component comp) {
        ...
        checkValue(ejs,comp, isAdvancedField || comp==eventStepField || comp==delayListField);
...
//          if (prelimButton.getIcon()!=null) prelimButton.setIcon(null);
    ...
    String def = sysRes.getString("EquationEditor.DefaultSolver");
...
//    solverPanel.add(solverHelpButton,BorderLayout.EAST);
    ...
    estimateFirstStepCombo = new JComboBox();
    ...
    accIndVelocityCombo = new JComboBox();
    ...
    forceSynchroCombo = new JComboBox();
...
//    forceSynchroPanel.add(solverHelpButton, BorderLayout.EAST);
    ...
    useBestInterpolationCombo = new JComboBox();
...
//    errorEditor = new CodeEditor (ejs, null,true);
//    errorEditor.setName(res.getString("EquationEditor.ErrorCode"));
//    errorEditor.setColor(myColor);
//    errorEditor.setFont (myFont);
//    errorEditor.addDocumentListener(new DocumentListener(){
...
//    parametersFullPanel.addTab(res.getString("EquationEditor.QSSInformation"),matrixFinalPanel);
    ...
    advancedButton = new JButton(res.getString("EquationEditor.Parameters"));
...
//    defBackground = eventField.getBackground();
    ...
    bottomLeftPanel.add(solverPanel,BorderLayout.WEST);
    bottomLeftPanel.add(absoluteTolerancePanel,BorderLayout.CENTER);
...
//FKH021024  public boolean isActive () { return table.isEnabled (); }
...
//      }
    ...
    if (_type==Editor.GENERATE_CHANGE_ENABLED_CONDITION) {
      StringBuffer code = new StringBuffer();
      code.append("    if (\""+getName()+"\".equals(_pageName)) { _pageFound = true; _isEnabled_" + generateName + " = _enabled; _resetSolvers(); } // Change enabled condition for " + _info + "." + getName()+ "\n");
...
//    if (!isActive()) return new StringBuffer();
    ...
    if (_type==Editor.GENERATE_DECLARATION) {
      return new StringBuffer("    if (_isEnabled_" + generateName+") _ODEi_" + generateName + ".step();\n");
    }
  ...
  public void addToInterpreter (Interpreter _interpreter) throws bsh.EvalError {
    String indVar = independentField.getText().trim();
    String inc = incrementField.getText().trim();
    if (indVar.length()<=0 || inc.length()<=0) return;
    EquationCode.addToInterpreter(ejs,this,indVar,_interpreter);
  }
    ...
    if (sai.getInteger()>0) {  // there are arrays
      ...
      for (int i=0,num=variableList.size(); i<num; i++) { // Rate equations
        ...
        if (eqnVar.isArray()) { // find the index string, if any
          indexStr = OsejsCommon.getPiece(eqnVar.getStateString(),"[","]",false);
          if (indexStr==null || indexStr.length()<=0) indexStr = null;
          if (indexStr==null) txt.append(" for (int _i=0; _i<"+varName+".length;_i++) _doubleArray[__c++] = "+varValue+";");
          else txt.append(" for (int "+indexStr+"=0; "+indexStr+"<"+varName+".length; "+indexStr+"++) _doubleArray[__c++] = "+varValue+";");
          txt.append(" // pre-initial value for "+ varName+"\n");
...
//      list.add(new ParametersSearchResult(_info+"."+addInfo,result.getTextFound(),errorEditor.getTextComponent(), result.getLineNumber(),result.getCaretPosition()));
    ...
    if (zenoEditor!=null) list.addAll(zenoEditor.search(_info,_searchString,_mode));
    return list;
  }
...
//    reading = false;
  ...
  private void editZenoEffect (boolean show) {

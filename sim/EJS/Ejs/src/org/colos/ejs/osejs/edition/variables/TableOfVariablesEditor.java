...
public class TableOfVariablesEditor implements Editor {
  ...
  static private final String[] COLUMN_NAMES = {"Name", "Value", "Type", "Dimension"};
  ...
  public TableOfVariablesEditor (Osejs _ejs) {
    ...
    final JPopupMenu popupMenu = new JPopupMenu(); // The popup menu for the table
...
//    fullPanel.setBorder (new EmptyBorder(10,0,0,0));
  ...
  public void setFont (Font _font) {
    for (int i=0; i<COLUMN_NAMES.length; i++) {
      if (i==TYPE_COLUMN) continue; // Not this one
  ...
  public StringBuffer generateCode (int _type, String _info) {        
    ...
    for (int row=0,n=variablesList.size(); row<n; row++) {
      ...
      if (_type==Editor.GENERATE_LIST_VARIABLES) {
        ...
        lineBuf.append(":"+varName+" : "+type+dimension+" : "+varData.comment+"\n");
      }
      else if (_type==Editor.GENERATE_DECLARATION) {
        lineBuf.append("  public " + type +" "+varName+" ");
        if (dim>0) {
          for (int k = 0; k < dim; k++) lineBuf.append("[]");
        }
        else {
        ...
        if (dim==0) {
          ...
          lineBuf.append("    "+varName+VariablesEditor.initCodeForAnArray (comment, lineOfIndexes, varName, type, dimension, value,ejs));
        }
      }
...
//      if (value!=null) System.out.println ("Value of "+name+" is "+value.getClass()+" = "+value.toString());
      varData.correctSyntax = TableOfVariablesEditor.isValidType(value,type,dimension);
...
//    if (ejs.isReading()) return false;
   ...
   */
  static public boolean isValidType (Value value, String type, String _dimension) {
  ...
  private void addEmptyRow (boolean before) {
    ...
    ListSelectionModel lsm = table.getSelectionModel();
    int min = lsm.getMinSelectionIndex(), max = lsm.getMaxSelectionIndex();
    if (max>=0 && max<variablesList.size()) varData.data[TYPE_COLUMN] = variablesList.get(max).data[TYPE_COLUMN];

...
public class HtmlEditor implements Editor, org.colos.ejs.library.utils.LocaleListener {
  ...
  protected boolean activeEditor=true; //FKH 021024
  ...
  static public File getCssFile(Osejs _ejs) { 
    File cssFile = new File (_ejs.getConfigDirectory(),"_ejs_library/css/ejsPage.css");
    if (!cssFile.exists()) cssFile = new File (_ejs.getBinDirectory(),"config/_ejs_library/css/ejsPage.css");
    return cssFile; 
  }
  ...
  public HtmlEditor (Osejs _ejs, boolean _editable, DescriptionEditor aParentEditor) {
    ...
    editButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) { 
        if (currentPage.isExternal()) {
          File htmlFile = ejs.chooseFileUnderSource(parentEditor.getChooser(), mainPanel,true);
    ...
    linkedButton.addItemListener(new ItemListener() {
      public void itemStateChanged(ItemEvent evt) { currentPage.setExternal(!currentPage.isExternal()); }
    });

    JPanel buttonsPanel = new JPanel(new GridLayout(1,0,0,0));
...
//    for (OneHtmlPage ohp : translations.values()) ohp.refresh(_hiddensToo);
//  }
...
//      } // end of for Entry
      ...
      for (Entry<LocaleItem,OneHtmlPage> entry : translations.entrySet()) {
        OneHtmlPage page = entry.getValue();
        String htmlCode = page.getCode();
        if (page.isExternal()) code.append(htmlCode+";");
        else {
          ...
          while (index>=0) {
            ...
            int srcEnd = index+tag.indexOf("\"",srcBegin+5);
            srcBegin = index + srcBegin + 5;
            // make the resource relative
            code.append(convertToRelativePath(ejs,htmlCode.substring(srcBegin,srcEnd)+ ";"));
            // Search next tag

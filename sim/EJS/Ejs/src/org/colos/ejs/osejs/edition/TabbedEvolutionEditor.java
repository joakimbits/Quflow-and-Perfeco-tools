...
public class TabbedEvolutionEditor extends TabbedEditor {
  ...
  protected void customMenuItems (java.awt.event.ActionListener actionListener) {
    ...
    addPage2  = createMenuItem ("addPageODE",defaultHeader,new ActionListener() {
      public void actionPerformed (ActionEvent _evt) {
        ...
        if (txt.length()>0) addPage (Editor.ODE_EDITOR,txt,null,true);
        else addPage (Editor.ODE_EDITOR,defaultString, null,true);
  ...
  public void addToInterpreter (Interpreter _interpreter) throws bsh.EvalError {
    for (Editor page : getPages()) if (page instanceof EquationEditor) ((EquationEditor) page).addToInterpreter(_interpreter);
  }

...
public class CreationPanelRow {
  static private final Hashtable<String,ImageIcon> iconTable = new Hashtable<String,ImageIcon>(); // The cache of icons already loaded
   ...
   * @return ImageIcon
   */
  static ImageIcon getElementIcon (String _elementType) {
    ...
    ImageIcon icon = ResourceLoader.getIcon(iconName);
    if (icon!=null) { 
  ...
  public CreationPanelRow (org.colos.ejs.osejs.Osejs _ejs) {
    ...
    ImageIcon selectedIcon = ResourceLoader.getIcon(sysRes.getString("Tree.Create.Icon"));
    Font font = InterfaceUtils.font(null,res.getString("Editor.TitleFont"));
    selectedCursor = Toolkit.getDefaultToolkit().createCustomCursor(selectedIcon.getImage(),new Point(5,1),"Create");
...
//    else mainPanel.setPreferredSize(new Dimension (res.getInteger("View.CreationPanelWidth"),0));
  ...
  private class ElementGroup {
    ...
    ElementGroup (TwoStrings _groupPair, ArrayList<TwoStrings> _groups) {
      ...
      for (int i=0; i<element.length; i++) {
        ...
        if (iconName!=null) {
          ImageIcon groupIcon = ResourceLoader.getIcon(iconName);
   ...
   */
                ...
                fileChooser.setFileView(new javax.swing.filechooser.FileView() {
                  ...
                  public Icon getIcon(File f) {
                    if (f.getName().endsWith(".gif")) {
                      ImageIcon imageIcon  = ResourceLoader.getIcon(f.getAbsolutePath());
    ...
    private JComponent addOneElement (final String _elementName) {
      ...
      ImageIcon theIcon = ResourceLoader.getIcon(icon);
      JLabel button;

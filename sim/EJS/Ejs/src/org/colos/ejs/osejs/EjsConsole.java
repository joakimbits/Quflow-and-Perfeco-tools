...
public class EjsConsole implements ConsoleServer {
  ...
  private JComboBox languageCombo, lookandfeelCombo, updateCombo;
  ...
  private String compressPrefix = "ejs_", zipPrefix="ejs_";
...
//    System.setProperty("Xdock:", "name=EJS Console");
    ...
    Dictionary<String,String> options = readOptions(optionsFile);
    ...
    try { // See if it is already running
      ...
      ConsoleServer stub = (ConsoleServer) reg.lookup("EjsConsoleServer");
  ...
  static private Dictionary<String,String> readOptions (File _file) {
    if (!_file.exists()) {
    ...
    options.put("LookAndFeel", OSPRuntime.DEFAULT_LF);
  ...
  static private void readOptionLine (String _line, Dictionary<String,String> options) {
    if      (_line.startsWith("JavaRoot=")) options.put("JavaRoot", _line.substring(9));
      ...
      if (_line.indexOf("true")>=0) options.put("LoadLastFile","true");
    }
...
*/
    ...
    // Logo 
    ImageIcon logoIcon = ResourceLoader.getIcon(sysRes.getString("EjsConsole.EjsLogo"));
    JLabel logoLabel = new JLabel(logoIcon);
    logoLabel.setBorder(new EmptyBorder(7,10,0,10));
    logoLabel.setVerticalAlignment(SwingConstants.TOP);
    JLabel versionLabel = new JLabel("Ejs "+org.colos.ejs.library._EjsConstants.VERSION,SwingConstants.CENTER);
    versionLabel.setVerticalAlignment(SwingConstants.TOP);
    versionLabel.setFont(versionLabel.getFont().deriveFont(15f));
    ...
    updateButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent evt) { checkForUpdates(EjsConsole.ALWAYS, tabbedPanel); }
    });
    ...
    optionsGridLeftPanel.add(updateButton);
    ...
    optionsGridBothPanel.add(optionsGridLeftPanel,BorderLayout.WEST);
    optionsGridBothPanel.add(optionsGridRightPanel,BorderLayout.CENTER);
...
//    advancedPanel.add(externalDirPanel);
    ...
    frame = new JFrame(res.getString("EjsConsole.Title")+" "+org.colos.ejs.library._EjsConstants.VERSION);
...
//    frame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
    ...
    Rectangle bounds = getScreenBounds(defaultScreen);
...
//    System.out.println ("bounds x = "+bounds.x);
   ...
   * @return
   */
  static private Rectangle getScreenBounds (int _screen) {
  ...
  private void checkUserDir () {
    ...
    //userDir = new File(initDir,OsejsCommon.USER_DIR_PATH);
    //userDir.mkdirs();
    //userDirField.setText(FileUtils.getPath(userDir));
    
    // This hangs the computer very often (but not always) under Vista
    userDirField.setText("");
    ...
    if (wsDir.exists()) chooser.setSelectedFile(wsDir);
    ...
    accPanel.add(checkBox,BorderLayout.SOUTH);
    ...
    while (!done) {
      ...
      if (option == JFileChooser.APPROVE_OPTION) { // This is where the computer may hang
        ...
        if (! (dirChosen.exists() && dirChosen.isDirectory() && FileUtils.isWritable(dirChosen) ) ) {
          ...
          if ((new File(dirChosen.getParent(),OsejsCommon.CONFIG_DIR_PATH)).exists() && 
            ...
            int confirmation = JOptionPane.showConfirmDialog(frame,res.getString("EjsConsole.ParentSeemsWorkspace"),
                res.getString("Warning"),JOptionPane.YES_NO_OPTION);
            correctDir =  (confirmation==JOptionPane.YES_OPTION);
          }
          else correctDir = true;
        }
        if (correctDir) {
          userDir = dirChosen;
          userDirField.setText(FileUtils.getPath(userDir));
    ...
    if (checkBox.isSelected() && !userDir.equals(wsDir)) copyDirectory (wsDir,userDir); // Copy the distribution workspace to the new workspace
  }

  /**
   * Copy all files in the source directory to the destination directory
   */
  private void copyDirectory (File _sourceDir, File _destDir) {
    boolean noToAll=false, yesToAll=false;
    ResourceUtil jarRes = null;
    Object[] ynOptions = null;
    String basePath = FileUtils.getPath(_sourceDir);
    
    boolean isMac = OSPRuntime.isMac();
    for (File source : JarTool.getContents(_sourceDir)) {
      if (isMac && source.getName().equals(".DS_Store")) continue;
  ...
  private void applyOptions (Dictionary<String,String> options) {
    ...
    try {
      ...
      Rectangle bounds = getScreenBounds(screen);
   ...
   * @param _filename
   */
  private void runEjs (String _filename) { runEjs(_filename,null); }
  ...
  private void runEjs (String _filename, String _password) {
    ...
    File toolsFile = new File(javaRoot + "/lib/tools.jar");
    if (toolsFile.exists()) { // It is a SDK
      JavaLib = javaRoot + "/lib/tools.jar" + File.pathSeparator +
                javaRoot + "/jre/lib/rt.jar";
    }
    else EjsLib += OsejsCommon.BIN_DIR_PATH+"/comSun.jar" + File.pathSeparator; // It is a JRE
    // Common stuff
    EjsLib += OsejsCommon.BIN_DIR_PATH+"/ejs.jar";
    ...
    if (OSPRuntime.isMac()) { // Under Mac OSX
      JavaLib = "";
    ...
    String EjsLibraries="";
    if (JavaLib.length()>0) EjsLibraries = JavaLib + File.pathSeparator;
    EjsLibraries +=  EjsLib + File.pathSeparator + getLocaleClasspath();
    ...
    if (_filename!=null) {
      if (justCompiling) {
        if (compressAfterCompilation) {
          if (compressPrefix.length()>0) { command.add("-prefix"); command.add(compressPrefix); }
          command.add("-jar");
        }
        if (zipAfterCompilation) {
          if (zipPrefix.length()>0) { command.add("-zip_prefix"); command.add(zipPrefix); }
          command.add("-zip");
        }
        command.add("-compile");
      }
    ...
    EjsConsole.GeneratedUtil runnable = new EjsConsole.GeneratedUtil(command.toArray(new String[0]),this);
...
//            if (subFile.getName().toLowerCase().endsWith(".xml")) 
    ...
    if (filesToProcess.size()>0) runEjs(filesToProcess.get(0));
  }
    ...
    for (Class<?> theClass : classes) {
      try {
        ...
        for (File file : JarTool.getContents(modelFile.getParentFile())) {
          ...
          if (ts.getSecondString().equals("html") && ts.getFirstString().startsWith(prefix)) {
...
*/
   ...
   */
  static private void checkForUpdates (String _whenToDoIt, Component _parent) {
...
//      System.out.println ("Last available version is "+versionTxt);
      ...
      if (tkn.nextToken().equals("EJS")) {
        ...
        buffer.append(res.getString("EjsConsole.InstalledRelease")+" "+org.colos.ejs.library._EjsConstants.VERSION +", build "+org.colos.ejs.library._EjsConstants.VERSION_DATE+".\n");
        buffer.append(res.getString("EjsConsole.CurrentRelease")  +" "+versionMajor+", build "+versionMinor+".\n\n");

        int resultMajor = versionMajor.compareTo(org.colos.ejs.library._EjsConstants.VERSION); 
        if  (resultMajor<0) {
          comparison = -1; 
          ...
          int resultMinor = versionMinor.compareTo(org.colos.ejs.library._EjsConstants.VERSION_DATE);
          if (resultMinor<0) {
        ...
        if (comparison>0) {
          Object[] options = { res.getString("EjsConsole.GotoEJSWeb"), res.getString("EjsConsole.UseCurrentVersion") };
          ...
          if (option==0) org.opensourcephysics.desktop.OSPDesktop.displayURL("http://"+OsejsCommon.EJS_SERVER+"/Main/Download");
          if (option!=1) System.exit(0);  
//          return "http://"+OsejsCommon.EJS_SERVER+"/uploads/Download/"+versionTxt+".zip";
        }
        else if (_parent!=null) { // Only if the update button was pressed explicitly
  ...
  class MyMouseListener extends MouseAdapter {
    public void mousePressed(MouseEvent evt) {
      ...
      if (aCmd.equals("javarootfield")) isJavaRootCorrect();
        ...
        JFileChooser chooser=OSPRuntime.createChooser(res.getString("View.FileDescription.xmlfile"), new String[]{"xml","ejs"});
...
//    private byte[] buf=new byte[max];
    ...
    public GeneratedUtil (String[] _command, EjsConsole _launcher) {
      command = _command;
      launcher = _launcher;
    }
...
//        launcher.outputArea.append (" in dir = "+launcher.initDir.getAbsolutePath());
    ...
    // ---- Inner class to read println's from generated applications as a separate Thread
  static private class GeneratedOutput implements Runnable {

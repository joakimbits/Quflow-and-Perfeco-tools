...
public class TreeOfElements {//implements InteractiveMouseHandler {
  ...
  private Osejs ejs;
  Icon linkIcon=null, editIcon=null, writeIcon=null, actionIcon=null, rootIcon=null, captureIcon=null;
...
//  private Cursor currentCursor;
  ...
  public TreeOfElements (Osejs _ejs, CreationPanelRow _cp) {
    ...
    editIcon = org.opensourcephysics.tools.ResourceLoader.getIcon(sysRes.getString("View.Edit.Icon"));
    writeIcon = org.opensourcephysics.tools.ResourceLoader.getIcon(sysRes.getString("View.Write.Icon"));
    linkIcon  = org.opensourcephysics.tools.ResourceLoader.getIcon(sysRes.getString("View.Link.Icon"));
    actionIcon  = org.opensourcephysics.tools.ResourceLoader.getIcon(sysRes.getString("View.Action.Icon"));
    rootIcon  = org.opensourcephysics.tools.ResourceLoader.getIcon(sysRes.getString("Tree.Main.Icon"));
    captureIcon  = org.opensourcephysics.tools.ResourceLoader.getIcon(sysRes.getString("Tree.Capture.Icon"));
...
//          else
    ...
    hiddenButton = new JCheckBox(res.getString("Tree.KeepPreviewHidden"),false);
    hiddenButton.addItemListener(new ItemListener(){
      public void itemStateChanged(ItemEvent e) {
        keepHidden = e.getStateChange()==ItemEvent.SELECTED;
        ControlWindow.setKeepHidden(keepHidden);
...
//  public void showDeprecatedFields() { if (showDeprecatedFields) traverseShowDeprecated(rootNode); }
  ...
  public void clear() {
    ...
    control.setValue("_Ejs_ShowDeprecated_",false);
    mainWindowNode=null;
    hiddenButton.setSelected(keepHidden = false);
    setChanged(false);
  }
...
//    tree.setEditable(_edit);
  ...
  public void readString (String _input, String _blockName) {
...
//      String visible    = TabbedEditor.getPiece(piece,"<Visible>","</Visible>",false);
      ...
      if (pasteList!=null) { // Check validity of parent and child
        ...
        if (!viewOf(parent).acceptsChild(viewOf(child))) {
          ...
          if (viewOf(grandParent).acceptsChild(viewOf(child))) parent = grandParent;
          else {
            JOptionPane.showMessageDialog(getComponent(),res.getString("Tree.ParentDoesNotAcceptThis"),
                                          res.getString("Tree.ErrorMessage"),JOptionPane.ERROR_MESSAGE);
            return;
...
//  private void add (String _classname, String _name) { add (currentNode,_classname,_name,getComponent()); }
  ...
  private void readCompound (DefaultMutableTreeNode _parent, String _classname) {
    ...
    if (fragment!=null) {
      pasteString(_parent,fragment);
      return;
    }
    ejs.readString((File) null,input,true);
  }

  private void add (DefaultMutableTreeNode _parent, String _classname, String _name, Component _component) {
...
//      if (viewOf(node).getElement().getComponent()!=null) // This causes no harm
    ...
    if (viewOf(node).isWindow() && _parent!=rootNode) {
      ...
      ((ControlWindow) viewOf(node).getElement()).dispose();
...
//   System.out.println ("Adding "+_child +" to "+_parent);
   ...
   if (currentNode!=null )
    if (isJMenuBar && (viewOf(_parent).getElement() instanceof ControlWindow)) { // Check that it doesn't have already a menubar
      ...
      if (!choosePosition(_parent, _child, null)) return; // No free slot, null for "use ANY free slot as default"
    }
    if (_position<0) treeModel.insertNodeInto (_child, _parent, _parent.getChildCount());
...
//    DataFlavor flavor = DataFlavor.stringFlavor;
  ...
  private void pasteString (DefaultMutableTreeNode _node, String _text) {
...
//      else prefix = "<Visible>false</Visible>\n";
  ...
  private StringBuffer traverseGenerateCode (DefaultMutableTreeNode _node, int _type) {
    codeCounter++;
    StringBuffer txt = new StringBuffer();
    if (_type==Editor.GENERATE_CODE && codeCounter%50==0) {
      ...
      if (OSPRuntime.isPopupTrigger(_e)) { //SwingUtilities.isRightMouseButton(_e)) {
        ...
        if (_e.getClickCount()==1) {
          if (creationPanel.getClassToCreate()!=null) {
            ...
            add (node,creationPanel.getClassToCreate(),getUniqueName(creationPanel.getNameToCreate()),viewOf(node).getElement().getVisual());
...
//        creationPanel.clear();
        ...
        if (_e.getClickCount()>1) {
          ...
          boolean discardAfterCreation = !_e.isAltDown();
          add (currentNode,creationPanel.getClassToCreate(),getUniqueName(creationPanel.getNameToCreate()),getComponent());
          if (discardAfterCreation) creationPanel.clear(); // disable the magic wand after creation
        }
      }
    }

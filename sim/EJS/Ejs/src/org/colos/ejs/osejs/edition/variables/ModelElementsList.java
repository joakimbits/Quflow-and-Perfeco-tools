...
public class ModelElementsList extends JScrollPane implements DropTargetListener, DragSourceListener, DragGestureListener, 
  ...
  private Osejs ejs;
  private MyDefaultListModel listModel;
  private JList list;
  ...
  public ModelElementsList(Osejs _ejs) {
    this.ejs = _ejs;
    listModel = new MyDefaultListModel();
    ...
    list.addMouseListener(new java.awt.event.MouseAdapter () {
      public void mousePressed (java.awt.event.MouseEvent evt) {
        ...
        if (OSPRuntime.isPopupTrigger(evt)) {
          ...
          ModelElementInformation elementInfo = (ModelElementInformation) listModel.elementAt(index);
  ...
  public void reportChange (org.colos.ejs.model_elements.ModelElement _element) {
    for (int i=0, n=listModel.getSize(); i<n; i++) {
      ModelElementInformation info = (ModelElementInformation) listModel.elementAt(i);
  ...
  public boolean nameExists(String _name) {
    for (int i = 0, n=listModel.getSize(); i<n; i++) {
      ModelElementInformation elementInfo  = (ModelElementInformation) listModel.getElementAt(i);
  ...
  public java.util.List<ModelElementInformation> getElements() {
    ...
    for (int i=0, n=listModel.getSize(); i<n; i++) {
      elementsList.add((ModelElementInformation) listModel.elementAt(i));
    }
    return elementsList;
  }
...
//  }
  ...
  public void drop(DropTargetDropEvent dtde) { 
    ...
    if (previousPosition<0) { // This is an addition from the elements panel
      String newName = getNameForElement(selection.getElement().getGenericName(),null);
      ...
      if (newName!=null) { // Create a new element
        ...
        ModelElementInformation elementInfo = ejs.getModelEditor().getElementsEditor().getPalette().instantiateModelElement(classname,selection.getJarFile(),false);
        if (elementInfo==null) dtde.dropComplete(false);
        else {
          elementInfo.setName(newName);
          addElement(elementInfo,index);
  ...
  private String getNameForElement (String _genericName, String _proposedName) {
    if (_proposedName==null) { // A new addition, requires a brand new name
      _proposedName = (String) JOptionPane.showInputDialog(this,Osejs.getResources().getString("Tree.ProvideAName"),
          Osejs.getResources().getString("Tree.NameTitle"),JOptionPane.QUESTION_MESSAGE,null,null,getUniqueName(org.colos.ejs.osejs.OsejsCommon.firstToLower(_genericName)));
    }
...
//        renumberElements();
  ...
  private class MyDefaultListModel extends DefaultListModel {

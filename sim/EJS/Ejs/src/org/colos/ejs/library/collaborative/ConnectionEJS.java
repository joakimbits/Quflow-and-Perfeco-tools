	...
	*/
	public ConnectionEJS(Socket con){
		...
		try {
			...
			System.err.println("Error in the input Stream "+e.getMessage());
			this.ok = false;
		}
		if(this.ok){
			...
			try {
				...
				if(ds.getSignal().equals("ID"))
				{
					...
					this.ok = false; //The data have been extracted incorrectly
				
			}catch (IOException e) {
				System.err.println("Error in the Object reading of the Stream "+e.getMessage());
				this.ok = false;
			}catch (ClassNotFoundException e) {
				System.err.println("Error ClassNotFoundException in the identification of the Stream "+e.getMessage());
				this.ok = false;
			} 
		}
	}

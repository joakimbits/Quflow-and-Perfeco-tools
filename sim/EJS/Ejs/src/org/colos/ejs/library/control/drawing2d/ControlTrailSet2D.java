...
public class ControlTrailSet2D extends ControlSet2D implements org.colos.ejs.library.control.swing.NeedsPreUpdate,
  ...
  public void reset () { // Overwrites default reset
    for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).clear();
  }

  public void initialize () { // Overwrites default initialize
    for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).initialize();
  }
...
//------------------------------------------------
  ...
  public ControlElement setProperty(String _property, String _value) {
    ...
    if (_value!=null) {
      ...
      if      (_property.equals("inputX")) for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setXLabel(useDefaultTitle ? "Input X" : _value);
      else if (_property.equals("inputY")) for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setYLabel(useDefaultTitle ? "Input Y" : _value);
    }
    ...
    switch (_index) {
      case 0 : 
        if (_value.getObject() instanceof int[]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementTrail)elements[i]).setMaximumPoints(val[i]);
        }
        else {
          int val = _value.getInteger();
          for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setMaximumPoints(val);
        }
        break;
      case 1 : 
      ...
      case 4 : 
        if (_value.getObject() instanceof int[]) {
          int[] val = (int[]) _value.getObject();
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementTrail)elements[i]).setSkipPoints(val[i]);
        }
        else {
          int val = _value.getInteger();
          for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setSkipPoints(val);
        }
        break;
      case 5 :
        if (_value.getObject() instanceof boolean[]) {
          boolean[] val = (boolean[]) _value.getObject();
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementTrail)elements[i]).setActive(val[i]);
        }
        else {
          boolean val = _value.getBoolean();
          for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setActive(val);
        }
        break;
      case 6 :
        if (_value.getObject() instanceof boolean[]) {
          boolean[] val = (boolean[]) _value.getObject();
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementTrail)elements[i]).setNoRepeat(val[i]);
        }
        else {
          boolean val = _value.getBoolean();
          for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setNoRepeat(val);
        }
        break;
      case 7 :
        if (_value.getObject() instanceof boolean[]) {
          boolean[] val = (boolean[]) _value.getObject();
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementTrail)elements[i]).setClearAtInput(val[i]);
        }
        else {
          boolean val = _value.getBoolean();
          for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setClearAtInput(val);
        }
        break;
      ...
      case 8 : 
        if (_value.getObject() instanceof String[]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementTrail)elements[i]).setXLabel(val[i]);
        }
        else {
          String label = _value.getString();
          for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setXLabel(label); 
        }
        break;
      case 9 : 
        if (_value.getObject() instanceof String[]) {
          String[] val = (String[]) _value.getObject();
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementTrail)elements[i]).setYLabel(val[i]);
        }
        else {
          String label = _value.getString();
          for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setYLabel(label);
        }
        break;
  ...
  public void setDefaultValue (int _index) {
    switch (_index) {
      case 0 : for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setMaximumPoints(0); break;
      case 1 : for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setConnectionType(ElementTrail.LINE_CONNECTION); break;
      case 2 : for (int i=0; i<numElements; i++) x[i] = 0.0; break;
      case 3 : for (int i=0; i<numElements; i++) y[i] = 0.0; break;
      case 4 : for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setSkipPoints(0); break;
      case 5 : for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setActive(true); break;
      case 6 :  for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setNoRepeat(false); break;
      case 7 :  for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setClearAtInput(false); break;
      case 8 : for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setXLabel(null); break;
      case 9 : for (int i=0; i<numElements; i++) ((ElementTrail)elements[i]).setYLabel(null); break;

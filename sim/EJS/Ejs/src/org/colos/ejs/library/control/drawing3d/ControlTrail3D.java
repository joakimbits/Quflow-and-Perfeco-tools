    ...
    if (isSet) {
      if (xArray==null) {
        if (yArray==null) {
          ...
          else for (int i=0,n=zArray.length; i<n; i++) trail.addPoint(x,y,zArray[i]);
        }
        else {
          if (zArray==null) for (int i=0,n=yArray.length; i<n; i++) trail.addPoint(x,yArray[i],z);
          else for (int i=0,n=Math.min(yArray.length,zArray.length); i<n; i++) trail.addPoint(x,yArray[i],zArray[i]);
        }
      }
      else {
        if (yArray==null) {
          if (zArray==null) for (int i=0,n=xArray.length; i<n; i++) trail.addPoint(xArray[i],y,z);
          else for (int i=0,n=Math.min(xArray.length,zArray.length); i<n; i++) trail.addPoint(xArray[i],y,zArray[i]);
        }
        else {
          if (zArray==null) for (int i=0,n=Math.min(xArray.length,yArray.length); i<n; i++) trail.addPoint(xArray[i],yArray[i],z);
          else for (int i=0,n=Math.min(xArray.length,Math.min(yArray.length,zArray.length)); i<n; i++) trail.addPoint(xArray[i],yArray[i],zArray[i]);
        }
      }
    }
    isSet = false;

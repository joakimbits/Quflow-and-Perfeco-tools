...
public abstract class Simulation extends Animation implements LocaleListener {
  ...
  static private List<File> resourcesExtracted = new ArrayList<File>();
...
//      System.err.println ("Path set to "+_path);
  ...
  static public File extractToTempFile(String _resourceString) {
    int index = _resourceString.lastIndexOf('.');
    String suffix = index>=0 ? _resourceString.substring(index) : ".tmp";
    try {
      File destFile = File.createTempFile("ejs_tmp_", suffix);
...
// ---------------------------------------------------
   ...
   * @param _height
   */
  public void addDescriptionPage (String _htmlPage, int _width, int _height) {
//    if (isUnderEjs) return;
...
//        catch (Exception exc) { exc.printStackTrace(); }
        ...
        descriptionPagesList.add(new EditorAndScroll(_htmlPage,editorPane,editorScrollPane));
      } 
      catch (IOException ioExc) {
        System.err.println("Attempted to read a bad URL: " + htmlRes.getURL());
      }
    } 
    else System.err.println("Couldn't find description file: "+_htmlPage);
   ...
   * @param height
   */
  public void addDescriptionPage (String _title, String _htmlPage, int width, int height) { addDescriptionPage (_htmlPage, width, height); }
...
//      System.out.println ("DescriptionPages : Dir created : "+descTempDirectory.getAbsolutePath());
    }
    for (String filename : model._getClassEjsResources()) {
      if (extractToDirectory(filename,descTempDirectory,false)==null) return null;
    }
    extractToDirectory("/_ejs_library/css/ejsPage.css",descTempDirectory,false); // Failure to do this does not invalidate the process
...
//      System.out.println ("Path = "+path);
      ...
      if (_isApplet) {
        ...
        if (mustExtract) {
          File resource = requiresResourceFile (path);
...
// --------------------------------------------------------
  ...
  public boolean isEmersionConnected () {
    if (eMersion==null) return false;
    return eMersion.isConnected();
  }
  ...
  private boolean javascriptControlMode(boolean localMode){
    ...
    if(javascriptControledApplet==null || isEmersionConnected())return false;
    if(localMode){
  ...
  public void getPopupMenu (final Component _component, int _x, int _y, String _element) {
    ...
    if (popupMenu==null) {
      ...
      if (canAccessDisk) {
        ...
        snapshotMenu.add(new AbstractAction(getMenuText("ejs_res:MenuItem.SnapshotTool")){
        ...
        snapshotMenuItem = new JMenuItem (getMenuText("tools_res:MenuItem.Snapshot"));
...
//            System.err.println("System password is "+systemPassword);
            ...
            if (!(model._isApplet() || OSPRuntime.isLauncherMode()) && quit) System.exit(0);
          }
        });
      }
      if (!isUnderEjs && canAccessDisk && !model._isApplet()) popupMenu.add(new AbstractAction(Memory.getResource("CreateHTMLPage")){
        public void actionPerformed(ActionEvent e) { createHTMLpage(); }
      });
  ...
  public boolean saveImage (String _filename, String _element) {
    ...
    if (isEmersionConnected () || javascriptControlMode() ) {
      ...
      if (_filename==null) _filename = "default.gif";
      if(isEmersionConnected ())return eMersion.saveImage(_filename,"GIF image", bi)!=null;
 ...
 /* this part did not for for applet mode, modified previous code to co-exist with Emersion case
    // Standard case
    if (javascriptControlMode()) {// FKH 200600415 add javascriptControlMode() for javascript control applet mode
      java.io.ByteArrayOutputStream output = new java.io.ByteArrayOutputStream();
      if (!SnapshotTool.getTool().saveImage ("default.jpg", comp, output)) return false;
      javascriptControledApplet.setImageByteArray(output.toByteArray());
      return true;
    }
    else
*/
...
// --------------------------------------------------------
  ...
  public void processArguments (String[] _args) {
    LauncherApplet applet = model._getApplet();
    if (applet!=null) {
      /* already been done at initEmersion
      try {
        String s = applet.getParameter("init"); // FKH 20060417
        if(s != null) {
          _init_ = new MethodWithOneParameter(0, applet._model, s, null, null, applet);
          _init_.invoke(0, applet);
        }
        ...
        if(arg0 != null) readState(arg0,applet.getCodeBase());
      ...
      */
      ...
      if (_args != null && _args.length > 0) {
        for (int i = 0; i < _args.length; i++) {
          if (_args[i].toLowerCase().endsWith(".xml")) readState(resetFile = _args[i]);
          else if (_args[i].equals("-_initialState")) readState(resetFile = _args[++i]);
        }
      }
...
//        System.out.println ("Is eMersion connected");
        // Do nothing to check
      }
...
//        System.err.println ("Trying to read from Reso Load URL "+urlStr);
        ...
        catch (Exception _exc) { exists = false; } // Do not complain
...
//        System.err.println ("Trying to read "+_filename);
      ...
      if (_filename.toLowerCase().endsWith(".xml")) {  // Read XML
        ...
        if (urlConnection!=null) {
          ...
          else if (isEmersionConnected()) xmlString = eMersion.readXML(_filename);
          else in = reader;
        }
        if (in!=null) {
...
//        if (!xmlContainsData (xmlString)) return true; // Why not? I read it, but data was irrelevant.
        ...
        if (urlConnection!=null) in = urlConnection.getInputStream();
          ...
          byte[] data = eMersion.readBinary(_filename);
          if (data==null) return false;
          in = new java.io.ByteArrayInputStream(data);
        }
        else in = new java.io.FileInputStream (_filename);
      ...
      if (_varList==null) { // read all variables in the order they appear
        for (int i=0; i<fields.length; i++) {
          if (! (fields[i].get(model) instanceof java.io.Serializable))continue; // Ignore these
          if (control!=null) { // read xml
            Object object = control.getObject(fields[i].getName());
            if (object != null) fields[i].set(model, object);
          }
          else if (din!=null) fields[i].set(model, din.readObject()); // read binary
        }
  ...
  public boolean saveVariables (String _filename, List<String> _varList) {
    ...
    if (_filename==null) { // Choose a filename, preferably an ".xml" file
      ...
      if (_filename==null) return false;
      if (_filename.lastIndexOf('.')<_filename.length()-5) _filename += ".xml";
    }
    try {
      ...
      if (_filename.toLowerCase().endsWith(".xml")) { // Save an XML file
        control = new XMLControlElement(this.getClass());
      }
      else { // Save in binary form
        // FKH 20060415 add javascriptcontrolMode()
        if (javascriptControlMode() || _filename.startsWith("ejs:") || isEmersionConnected ()) out = new ByteArrayOutputStream ();
        else out = new java.io.FileOutputStream (_filename);
      ...
      if (control!=null) {  // Write the XML
        ...
        else if (isEmersionConnected()) return eMersion.saveXML(_filename, "XML file", control.toXML()) != null;
        ...
        if (out!=null) {
          ...
          else if (isEmersionConnected ()) return eMersion.saveBinary(_filename,"Binary data",( (ByteArrayOutputStream) out).toByteArray())!=null;
...
// --------------------------------------------------------
  ...
  public boolean saveText (String _filename, String _text) {
    try {
      ...
      if (_filename.startsWith("ejs:")) out = new java.io.CharArrayWriter ();
        ...
        if (_filename.toLowerCase().endsWith(".xml")) return eMersion.saveXML(_filename, "XML file", _text) != null;
        return eMersion.saveText(_filename, "Text file", _text) != null;
      }
  ...
  public String readText (String _filename) {
    return readText (_filename,(java.net.URL) null);
  }
  ...
  public String readText (String _filename, java.net.URL _codebase) {
    if (_filename.startsWith("url:")) {
      ...
      try { return (String) ( (new java.net.URL(url)).getContent()); }
      catch (java.lang.Exception ioe) {
    ...
    try { // Either memory or a file
      ...
      if (_filename.startsWith("ejs:")) in = new java.io.CharArrayReader((char[]) memory.get(_filename));
        ...
        if (_filename.toLowerCase().endsWith(".xml")) return eMersion.readXML(_filename);
        return eMersion.readText(_filename);
      }

  ...
  */
  final public void invokeActions (int _type) {
    if (myActiveState && isControlUpdatingSimulation()) {
      ...
      if (sim!=null) for (MethodWithOneParameter method : myActionsList) sim.invokeMethodWhenIdle(new MethodDelayedAction(method, _type, this));
...
//    if (myGroup!=null) myGroup.updateSimulation();
  ...
  */
  final public void variableChangedDoNotUpdate (int _variableIndex, Value _value) {
    ...
    if (myActiveState && isControlUpdatingSimulation()) {
      Simulation sim = getSimulation();
      if (sim!=null) for (MethodWithOneParameter method : myActionsList) sim.invokeMethodWhenIdle(new MethodDelayedAction(method, ControlElement.VARIABLE_CHANGED, this));

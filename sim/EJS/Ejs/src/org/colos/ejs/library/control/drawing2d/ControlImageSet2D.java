...
//------------------------------------------------
    ...
    switch (_index) {
      case 0 : 
        if (_value.getObject() instanceof String[]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementImage)elements[i]).setImageFile(val[i]);
        }
        else {
      ...
      case 1 : 
        if (_value.getObject() instanceof boolean[]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementImage)elements[i]).setTrueSize(val[i]);
        }
        else {
      ...
      case 2 : 
        if (_value.getObject() instanceof int[]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) elements[i].getStyle().setRelativePosition(val[i]);
        }
        else {
          int val = _value.getInteger();
          for (int i=0; i<numElements; i++) elements[i].getStyle().setRelativePosition(val);
        }
        break;
  ...
  public void setDefaultValue (int _index) {
    switch (_index) {
      case 0 : for (int i=0; i<numElements; i++) ((ElementImage)elements[i]).setImageFile(null); break;
      case 1 : for (int i=0; i<numElements; i++) ((ElementImage)elements[i]).setTrueSize(false); break;
      case 2 : for (int i=0; i<numElements; i++) elements[i].getStyle().setRelativePosition(Style.CENTERED); break; 

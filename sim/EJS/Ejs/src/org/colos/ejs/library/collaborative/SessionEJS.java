...
public class SessionEJS implements Runnable{
	...
	*/
	public SessionEJS(ListStudentsTree tree){
		...
		tree.txtArea.setText("Creando una sesión....");
		...
		this.sender = new Sender(this.myConnections, this.buffer);
		this.sender.start();
	...
	*/
	public synchronized void newConnectionSession(ConnectionEJS aux){
		...
		System.out.println("Adding a new socket client...");
		...
		if(this.myConnections.size()==0)
		{
			...
			{
				this.administrator = new sessionHandler(this, this.childrens, this.myConnections);
...
//		Si nos da problemas podemos poner this.myList.port en esta sentencia, porque realmente está esperando en el mismo puerto
		String _args[] = {aux.IAddress().getHostName(),Integer.toString(aux.connectionSocket().getLocalPort()),aux.user()};
		...
		//CJB110308
		//Si nos da problemas podemos poner this.myList.port en esta sentencia, porque realmente está esperando en el mismo puerto
		DatagramPacket auxPacket = new DatagramPacket(bufferUDPSend,bufferUDPSend.length,aux.IAddress(),aux.connectionSocket().getLocalPort());
		//CJB110308
		...
		ReadInputThread readerAux = new ReadInputThread(buffer,aux,this);
		...
		myList.setText("New client number "+counter+" and name "+aux.user());
	}
	...
	*/
	public synchronized void giveChalkConnection(String ident){
		...
		System.out.println("Giving the chalk at a connection...");
		...
		for(int i = 0; i < this.myConnections.size(); i++){
			if(this.myConnections.elementAt(i).user().equals(ident)){
				...
				} catch (IOException e) {System.err.println("Error in chalk "+e.getMessage());}
	...
	*/
	public synchronized void removeChalkConnection(String ident){
			...
			System.out.println("Removing the chalk of a connection...");
			...
			for(int i = 0; i < this.myConnections.size(); i++){
				if(this.myConnections.elementAt(i).user().equals(ident)){
					...
					} catch (IOException e) {System.err.println("Error removing the chalk "+e.getMessage());}
	...
	*/
	public synchronized void closeSession(){
		...
		//Print messages
		this.getMyList().setText("Clossing the session");
		System.out.println("Stoping the threads...");
		ConnectionEJS aux;
	...
	*/
	public void run(){
		System.out.println("Comienzo la escucha");
		String temp;
...
class sessionHandler extends Thread{
	...
	*/
	public sessionHandler(SessionEJS superior, Vector<ReadInputThread> readers, Vector<ConnectionEJS> clients){
		System.out.println("Creating a new session Administrator...");
	...
	*/
	public void run(){
		while (this.activeAdmin){
			for( int i = readers.size()-1; i>=0; i--){
				if(!(this.readers.elementAt(i).isActive()) && !(this.connections.elementAt(i).getChalk())){
					this.parent.getMyList().setText("Client disconnected "+this.connections.elementAt(i).user());
		...
		this.parent.getMyList().setText("Session finished");
	...
	public void run(){
		while(active){
			...
			//Exception Management
				...
				System.err.println("ERROR in SocketException "+e.getMessage());
				this.active = false;
			} catch (IOException e) {
				System.err.println("ERROR in IOException. Connection Closed "+e.getMessage());
				this.active = false;
			} catch (ClassNotFoundException e) {
				System.err.println("ERROR ClasNotFound exception "+e.getMessage());
				this.active=false;
			}
		}
		System.out.println("Getting out of the reader...");
		return;
	}

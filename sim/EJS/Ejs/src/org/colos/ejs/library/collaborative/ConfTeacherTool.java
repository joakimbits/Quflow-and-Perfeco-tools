...
public class ConfTeacherTool {
	...
	protected static Dimension defaultSize = new Dimension (470, 350);
	...
	*/
	...
	private void acceptAction(ActionEvent evt)
	{
        ...
        if(IPCBox.isSelected())
        	ListFields.add("local");
        else
        	ListFields.add("public");
		...
		if(CheckEmptyFields(ListFields))
			if(PasswordField.getText().equals("ejs"))
			{
			    ...
			    String dimensionFrame = Integer.toString(this.sim.getView().getComponent(_mainFrame).getSize().width-7);
			    ...
			    String response = petitionPost(new DataSend(ListFields));
			  
				if(response.equals("Error"))
					HtmlField.setText("No generated HTML");
				else
				{
	...
	*/
	...
	{		
		...
		teacherGUI = ListStudentsTree.getTool(sim,Integer.parseInt(PortField.getText()));

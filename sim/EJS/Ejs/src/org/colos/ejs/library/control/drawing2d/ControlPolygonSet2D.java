...
public class ControlPolygonSet2D extends ControlSet2D implements org.colos.ejs.library.control.swing.NeedsPreUpdate {
  ...
  public void preupdate() {
    if (dataSet) return;
    if (xArray==null) {
      if (yArray==null) for (int i=0; i<numElements; i++) ((ElementPolygon)elements[i]).setData(xFixedArray[i], yFixedArray[i]);
      else for (int i=0, n=Math.min(numElements,yArray.length); i<n; i++) ((ElementPolygon)elements[i]).setData(xFixedArray[i], yArray[i]);
    }
    else {
      if (yArray==null) for (int i=0, n=Math.min(numElements,xArray.length); i<n; i++) ((ElementPolygon)elements[i]).setData(xArray[i],yFixedArray[i]); 
      else for (int i=0, n=Math.min(numElements,Math.min(xArray.length,yArray.length)); i<n; i++)
    ...
    switch (_index) {
      case 0 : 
        if (_value.getObject() instanceof boolean[]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementPolygon)elements[i]).setClosed(val[i]);
        }
        else {
      ...
      case 1 : 
        if (_value.getObject() instanceof double[][][]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementPolygon)elements[i]).setData(val[i]);
        }
        else {
          double[][] val = (double[][]) _value.getObject();
...
//        selectedValue.value = set.indexInGroup((Element) _event.getSource());
        ...
        variableChanged (ELEMENT_SELECTED+getPropertiesAddedToSet(),selectedValue);

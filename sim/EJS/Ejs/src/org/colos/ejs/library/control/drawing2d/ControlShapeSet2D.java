    ...
    switch (_index) {
      case 0 : 
        if (_value.getObject() instanceof int[]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementShape)elements[i]).setShapeType(val[i]);
        }
        else {
      ...
      case 1 : 
        if (_value.getObject() instanceof boolean[]) {
          ...
          for (int i=0, n=Math.min(numElements,val.length); i<n; i++) ((ElementShape)elements[i]).setPixelSize(val[i]);
        }
        else {

...
public class EjsControl {
  ...
  static private GraphicsConfiguration defaultScreenGraphicsConfiguration = getGraphicsConfiguration(0);
   ...
   */
   public void dispose() {
     for (ControlElement element : elementList) {
      if (element instanceof ControlWindow) ((ControlWindow) element).dispose();
     }
   }
  ...
  */
  final public ControlElement addElement (ControlElement element, String name) {
  	if (replaceOwnerName!=null && replaceOwnerName.equals(name)) { // Manage replaceOwnerFrame
	    if (element.getObject() instanceof Frame) setOwnerFrame (replaceOwnerFrame);
...
//    System.out.println ("Calling update");
  ...
  */
  public void clear() {
    ...
    // It seems that, in 1.6, Dialgos need to be destroyed BEFORE their owner frames!
...
//      System.err.println("  Element to destroy "+element.toString()+"(class is "+element.getClass().getName()+")");
  ...
  private void destroy(ControlElement _element, boolean _informMyParent) {
    ...
    if (_element instanceof ControlWindow) ((ControlWindow)_element).dispose();
  }
...
//---------------------------------------------------
  ...
  public void alert(String _panel, String _title, String _message) {
    JOptionPane.showMessageDialog(getComponent(_panel), _message, _title, JOptionPane.INFORMATION_MESSAGE);
  }
...
//    JDialog dialog = new JDialog(windowOwner);
  ...
  /**
   * Opens a document using the native application. If the documentg is inside the JAR, it is extracted to a temporary directory
   * @param document can be a url, pdf, txt file, etc
   * @return
   */
  public boolean showDocument (String document) {
    Resource resource = ResourceLoader.getResource(document);
    if (resource==null) return false;
    if (resource.getFile()!=null) return OSPDesktop.open(resource.getFile());
    java.io.File file = Simulation.extractToTempFile (document);
//    System.out.println ("File is "+file.getAbsolutePath());
    return OSPDesktop.open(file);
  }

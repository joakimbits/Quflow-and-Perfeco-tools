...
public abstract class ControlSet2D extends ControlElement2D {
  ...
  protected void copyAnElement (Element oldElement, Element newElement) {
    ...
    newElement.setCanBeMeasured(oldElement.isMeasured());
...
// ------------------------------------------------
      ...
      switch (_index) {
        case POSITION_X : 
          ...
          if (_value.getObject() instanceof double[]) {
            ...
            for (int i=0, n=Math.min(numElements,val.length); i<n; i++) elements[i].setX(theXs[i] = val[i]);
          }
          else {
            double val = _value.getDouble();
            for (int i=0; i<numElements; i++) elements[i].setX(theXs[i]=val);
          }
          break;
        case POSITION_Y : 
          allThePos = null; // Not set using the POSITION property
          if (_value.getObject() instanceof double[]) {
            double[] val = (double[]) _value.getObject();
            if (!numberOfElements_isSet) checkNumberOfElements (val.length,false);
            for (int i=0, n=Math.min(numElements,val.length); i<n; i++) elements[i].setY(theYs[i] = val[i]);
          }
          else {
            double val = _value.getDouble();
            for (int i=0; i<numElements; i++) elements[i].setY(theYs[i]=val);
          }
          break;
        case POSITION :
          if (_value.getObject() instanceof double[][]) {
        ...
        case SIZE_X :
          ...
          if (_value.getObject() instanceof double[]) {
            double[] val = (double[]) _value.getObject();
            if (!numberOfElements_isSet) checkNumberOfElements (val.length,false);
            for (int i=0, n=Math.min(numElements,val.length); i<n; i++) elements[i].setSizeX((theSizeXs[i] = val[i])*allScalex);
          }
          else {
            double val = _value.getDouble();
            for (int i=0; i<numElements; i++) elements[i].setSizeX((theSizeXs[i]=val)*allScalex);
          }
          break;
        case SIZE_Y : // Not set using the SIZE property
          ...
          if (_value.getObject() instanceof double[]) {
            double[] val = (double[]) _value.getObject();
            if (!numberOfElements_isSet) checkNumberOfElements (val.length,false);
            for (int i=0, n=Math.min(numElements,val.length); i<n; i++) elements[i].setSizeY((theSizeYs[i] = val[i])*allScaley);
          }
          else {
            double val = _value.getDouble();
            for (int i=0; i<numElements; i++) elements[i].setSizeY((theSizeYs[i]=val)*allScaley);
          }
          break;
        case SIZE :
          if (_value.getObject() instanceof double[][]) {
        ...
        case SCALE_X : 
          ...
          for (int i=0; i<numElements; i++) elements[i].setSizeX(theSizeXs[i]*allScalex);
          break;
        case SCALE_Y : 
          allScaley = _value.getDouble(); 
          for (int i=0; i<numElements; i++) elements[i].setSizeY(theSizeYs[i]*allScaley);
          break;

        case TRANSFORMATION :
          if (_value.getObject() instanceof AffineTransform) {
            AffineTransform theTransform = (AffineTransform) _value.getObject();
            for (int i=0; i<numElements; i++) elements[i].setTransformation(theTransform);
          }
          else if (_value.getObject() instanceof double[]) {
            double[] array = (double[]) _value.getObject();
            for (int i=0,n=Math.min(array.length,numElements); i<n; i++) elements[i].setTransformation(AffineTransform.getRotateInstance(array[i]));
          }
            ...
            for (int i=0,n=Math.min(arrayTransform.length,numElements); i<n; i++) elements[i].setTransformation(arrayTransform[i]);
          }
          else {
        ...
        case VISIBLE :
          if (_value.getObject() instanceof boolean[]) {
            ...
            for (int i=0; i<numElements; i++) elements[i].setVisible(val);
          }
          break;
        case LINE_COLOR :
        ...
        case LINE_WIDTH :
          if (_value.getObject() instanceof double[]) {
            ...
            for (int i=0; i<numElements; i++) elements[i].getStyle().setLineWidth(val);
          }
          break;
        case FILL_COLOR :
          if (_value instanceof IntegerValue) {
        ...
        case DRAWING_FILL :
          if (_value.getObject() instanceof boolean[]) {
            ...
            boolean val = _value.getBoolean();
            for (int i=0; i<numElements; i++) elements[i].getStyle().setDrawingFill(val);
          }
          break;
        case DRAWING_LINES :
          if (_value.getObject() instanceof boolean[]) {
            ...
            boolean val = _value.getBoolean();
            for (int i=0; i<numElements; i++) elements[i].getStyle().setDrawingLines(val);
          }
          break;
        ...
        case MOVES_GROUP      :
            if (_value.getObject() instanceof boolean[]) {
              ...
              boolean val = _value.getBoolean();
              for (int i=0; i<numElements; i++) elements[i].getInteractionTarget(Element.TARGET_POSITION).setAffectsGroup(val);
            }
            break;
        case ENABLED_SIZE     :
          if (_value.getObject() instanceof boolean[]) {
        ...
        case RESIZES_GROUP    :
            if (_value.getObject() instanceof boolean[]) {
              ...
              boolean val = _value.getBoolean();
              for (int i=0; i<numElements; i++) elements[i].getInteractionTarget(Element.TARGET_SIZE).setAffectsGroup(val);
            }
            break;
        ...
        if (_value.getObject() instanceof int[]) {
          ...
          int val = _value.getInteger();
          for (int i=0; i<numElements; i++) elements[i].getStyle().setSensitivity(val);
        }
        break;
        ...
        case MEASURED : 
          if (_value.getObject() instanceof boolean[]) {
            ...
            boolean val = _value.getBoolean();
            for (int i=0; i<numElements; i++) elements[i].setCanBeMeasured(val);
          }
          break;
        case EXTRA_COLOR :
          if (_value instanceof IntegerValue) {
    ...
    public void setDefaultValue (int _index) {
      ...
      _index -= SET2D_ADDED; // So that to respect the super's numbering
      switch (_index) {
        case POSITION_X : for (int i=0; i<numElements; i++) elements[i].setX(theXs[i]=defaultElementX); break;
        case POSITION_Y : for (int i=0; i<numElements; i++) elements[i].setY(theYs[i]=defaultElementY); break;
        case POSITION :   allThePos = null; break;
          
        case SIZE_X : for (int i=0; i<numElements; i++) elements[i].setSizeX((theSizeXs[i]=defaultElementSizeX)*allScalex); break;
        case SIZE_Y : for (int i=0; i<numElements; i++) elements[i].setSizeY((theSizeYs[i]=defaultElementSizeY)*allScaley); break;
        ...
        case LINE_COLOR : for (int i=0; i<numElements; i++) elements[i].getStyle().setLineColor(defElementLines); break;
        case LINE_WIDTH : for (int i=0; i<numElements; i++) elements[i].getStyle().setLineWidth(1.0f); break;
        case FILL_COLOR : for (int i=0; i<numElements; i++) elements[i].getStyle().setFillColor(defElementFill); break;
        ...
        case MEASURED : for (int i=0; i<numElements; i++) elements[i].setCanBeMeasured(true); break;

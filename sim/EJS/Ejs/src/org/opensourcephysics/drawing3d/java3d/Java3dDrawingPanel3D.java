...
public class Java3dDrawingPanel3D implements ImplementingPanel{
  ...
  public void visualizationChanged(int _change){
    switch(_change){
      case VisualizationHints.HINT_BACKGROUND_IMAGE:
        setImage(panel3D.getVisualizationHints().getBackgroundImage(),panel3D.getVisualizationHints().getBackgroundTile());
        break;
  ...
  private void setImage(String _name, Dimension _tile){
    ...
    BufferedImage image = ResourceLoader.getBufferedImage(_name);
    width = (_tile==null)? canvas.getWidth() : _tile.width;
    height = (_tile==null)? canvas.getHeight() : _tile.height;
    AffineTransformOp op = new AffineTransformOp(AffineTransform.getScaleInstance((double)width/image.getWidth(),(double)height/image.getHeight()),optionsImage);
    ImageComponent2D image2D = new ImageComponent2D(ImageComponent.FORMAT_RGBA, op.filter(image,null));
    ...
    java.net.URL url= ResourceLoader.getResource(_name).getURL();
	TextureLoader tex = new TextureLoader(url, new String("RGBA"),TextureLoader.BY_REFERENCE | TextureLoader.Y_UP, null);

...
//  		startPlayingAtFrame(frameToPlay);
  ...
  private void load(String fileName) throws IOException { 	
    ...
    if (container.open(res.getAbsolutePath(), IContainer.Type.READ, null) < 0)
  ...
  private void resetContainer() {
    // seek backwards
    if (container.seekKeyFrame(-1,
        Long.MIN_VALUE, 0, Long.MAX_VALUE,
        IContainer.SEEK_FLAG_BACKWARDS)>=0) {
    	loadNextPacket();
    }
//    // if seeking doesn't work, reopen container
//  	String url = container.getURL();
//  	container.close();
//    container.open(url, IContainer.Type.READ, null);
//		loadNextPacket();
//  	if (packet.getTimeStamp() == startTimeStamp) {
//  		System.out.println("container reset by reopening");
//  		return;
//  	}  	

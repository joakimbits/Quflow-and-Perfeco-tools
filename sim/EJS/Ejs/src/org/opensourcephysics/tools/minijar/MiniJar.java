...
public class MiniJar {
  static private final String bootClassPath; // The Java boot class path as a String
   ...
   */
  static {
    bootClassPath = System.getProperty("sun.boot.class.path"); //$NON-NLS-1$
...
//    printStream.println ("Add desired "+pattern);
    if (pattern.toLowerCase().endsWith(".jar") || pattern.toLowerCase().endsWith(".zip")) { //$NON-NLS-1$ //$NON-NLS-2$
      File file = new File (pattern);
  ...
  private boolean isSystemClass (String path) {

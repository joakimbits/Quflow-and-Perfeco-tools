...
class VI(UserDict):
    ...
    def __getitem__(self, key):
        "self[key]"
        try:
            return self._vi.GetControlValue(key)
        except:
    ...
    def __setitem__(self, key, value):
        "self[key] = value"
        try:
            self._vi.SetControlValue(key, value)
        except:
...
if __name__ == '__main__':
    ...
    soundCheck = VI(r'LabVIEW\sound check.vi',
        ...
        drivers)
    print
    "soundCheck VI:", soundCheck
    print
    "soundCheck function:", soundCheck.__doc__
    print
    "soundCheck table:", soundCheck.keys()
    print
    "soundCheck attributes:", ", ".join(
        ["%s/%s/" % (k[0], k[1:]) for k in soundCheck.keys()])
    print
    print
    "soundCheck.Show () opens the panel"
    soundCheck.Show()
    f0 = 440
    for i in range(-3, 5):
        f = f0 * 2 ** i
        p = 30 * i
        print
        "soundCheck (freq=%g, phase=%g)" % (f, p), '->',
        print
        len(soundCheck(freq=f, phase=p)[2][0]), "wave samples"
    print
    "soundCheck.v == soundCheck.vol == soundCheck['volume'] ==", soundCheck.v
    print
    "soundCheck.duration = 1 runs the VI with 1 s duration"
    soundCheck.d = 1
    print
    print
    'You must execute ">>> del soundCheck" or close Python to edit the VI'
